...
#undef NGP_CUDA_HOST_DEVICE
    ...
    void run_validation(const std::uint8_t* validation_pixels, const float* validation_camera, std::uint32_t validation_frame_count, std::uint32_t width, std::uint32_t height, float focal_length, const std::uint8_t* occupancy, const std::uint32_t* grid_offsets, const std::uint16_t* params, std::uint32_t density_param_offset, std::uint32_t rgb_param_offset, std::uint32_t grid_param_offset, float* sample_coords, std::uint16_t* density_input, std::uint16_t* rgb_input, std::uint16_t* network_output, std::uint32_t* validation_numsteps, std::uint32_t* validation_sample_counter, std::uint32_t* validation_overflow_counter, double* validation_loss_sum, double& out_loss_sum);
