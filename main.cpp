    ...
    const auto result = ngp::dataset::load_nerf_synthetic("../data/nerf-synthetic/lego")
                            ...
                            .and_then([&](auto&& ngp) -> std::expected<void, std::string> {
                                ...
                                for (std::int32_t step = 0; step < total_steps; step += chunk_steps) {
                                    ...
                                    std::println("step={} loss={:.6f} chunk_ms={:.3f} steps/s={:.2f} rays={} samples={}/{}", stats->step, stats->loss, stats->elapsed_ms, static_cast<float>(chunk_steps) * 1000.0f / stats->elapsed_ms, stats->rays_per_batch, stats->measured_sample_count, stats->measured_sample_count_before_compaction);
                                }
